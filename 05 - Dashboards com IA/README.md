...
A causa está na measure `Taxa de Churn`: ela usa `COUNT` em vez de `SUM` no numerador — em vez de **somar** os cancelamentos, **conta todas as linhas** —, inflando o churn para 100%. Agora vem a grande vantagem de uma metric view: **conserte a regra uma única vez e todos os gráficos que a usam mudam juntos** — sem tocar em nenhum gráfico.
