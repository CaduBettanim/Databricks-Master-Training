...
```sql
CREATE OR REPLACE FUNCTION dbacademy.<seu_schema>.gerar_email_retencao(p_id STRING)
...
RETURN
    WITH dados AS (
        ...
        JOIN dbacademy.<seu_schema>.churn_scores cs ON dc.id_cliente = cs.id_cliente
...
O Genie Code calcula as regras do Passo 2 em SQL (`CASE`) e usa `ai_query` para redigir o e-mail.
...
SELECT
    email
FROM dbacademy.<seu_schema>.gerar_email_retencao('C00018');
```
